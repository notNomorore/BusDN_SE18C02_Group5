...
const { User, Route, MonthlyPass, WalletTransaction, Promotion } = require("../models");
const {
    getFareMatrix,
...
function applyPriorityDiscount(basePrice, discountPercent) {
    const b = Number(basePrice || 0);
    const d = normalizeDiscountPercent(discountPercent);
    const discounted = Math.round((b * (100 - d)) / 100);
...
exports.purchaseMonthlyPass = async (req, res) => {
    try {
        ...
        if (!month || month < 1 || month > 12) {
            return res.redirect(pageRedirectWithMsg("error", "Thang ap dung khong hop le.", backQuery));
        }

        if (!year || year < 2000 || year > 3000) {
            return res.redirect(pageRedirectWithMsg("error", "Nam ap dung khong hop le.", backQuery));
        }
        ...
        if (targetMonthStart < currentMonthStart) {
            return res.redirect(pageRedirectWithMsg("error", "Khong the mua ve cho thang da qua.", backQuery));
        }
        ...
        if (passType === PASS_TYPE.SINGLE_ROUTE) {
            route = await Route.findById(routeId).lean();
            if (!route || route.status !== "ACTIVE") {
                return res.redirect(pageRedirectWithMsg("error", "Tuyen khong hop le hoac da ngung hoat dong.", backQuery));
            }
        }
        ...
        if (passType === PASS_TYPE.SINGLE_ROUTE) duplicateFilter.routeId = routeId;

        const existingPass = await MonthlyPass.findOne(duplicateFilter).lean();
        if (existingPass) {
            const duplicateMsg = passType === PASS_TYPE.INTER_ROUTE
                ? `Ban da mua ve thang lien tuyen trong ${pad2(month)}/${year} roi.`
                : `Ban da mua ve thang cho tuyen nay trong ${pad2(month)}/${year} roi.`;
            return res.redirect(pageRedirectWithMsg("error", duplicateMsg, backQuery));
        }
        ...
        if (!Number.isFinite(basePrice) || basePrice <= 0) {
            return res.redirect(pageRedirectWithMsg("error", "Gia ve thang chua duoc cau hinh.", backQuery));
        }
        ...
        if (promoCode) {
            const reserved = await validateAndReservePromotion({
                promoCode,
                userId,
                passType,
                routeId,
            ...
            promoReserved = reserved.promotion;
            promoDiscountAmount = reserved.discountAmount;
        }

        const price = Math.max(1, priceAfterPriority - promoDiscountAmount);
        const orderCode = toOrderCode();
        const txnRefPrefix = paymentMethod === PAYMENT_METHOD.MOMO ? "MOMO-MP" : "VNPAY-MP";
        const txnRef = `${txnRefPrefix}-${orderCode}`;
        ...
        const tx = await WalletTransaction.create({
            ...
            direction: "OUT",
            txnType: "MONTHLY_PASS",
            note: `${paymentMethod} order for monthly pass ${pad2(month)}/${year}`,
            ...
            rawReturn: {
                ...
                basePrice,
                discountPercent,
                promoCode: promoReserved ? promoReserved.code : "",
                promotionId: promoReserved ? String(promoReserved._id) : "",
                promoDiscountAmount,
                promoReserved: Boolean(promoReserved),
                promoReleased: false
            }
        });
        ...
        if (paymentMethod === PAYMENT_METHOD.VNPAY) {
            ...
            if (!tmnCode || !hashSecret) {
                ...
                await WalletTransaction.updateOne({ _id: tx._id }, { $set: { status: "FAILED", note: "Missing VNPAY config." } });
                return res.redirect(pageRedirectWithMsg("error", "Thieu cau hinh VNPAY tren server.", backQuery));
            }
            ...
            const vnpParams = {
                ...
                vnp_CurrCode: "VND",
                vnp_TxnRef: txnRef,
                vnp_OrderInfo: `Thanh toan ve thang ${pad2(month)}/${year}`,
                vnp_OrderType: "other",
                vnp_Amount: price * 100,
                vnp_ReturnUrl: returnUrl,
                vnp_IpAddr: getClientIp(req),
                vnp_CreateDate: formatDateVnp(nowPay),
                vnp_ExpireDate: addMinutesVnp(nowPay, 15)
            };

            const paymentUrl = buildVnpUrl(vnpUrl, vnpParams, hashSecret);
            await WalletTransaction.updateOne({ _id: tx._id }, { $set: { rawIpn: { checkoutUrl: paymentUrl } } });
        ...
        if (paymentMethod === PAYMENT_METHOD.MOMO) {
            ...
            const redirectUrl = process.env.MOMO_MONTHLY_RETURN_URL || `${baseUrl}/passenger/passes/monthly/momo-return`;
            const ipnUrl = process.env.MOMO_MONTHLY_IPN_URL || `${baseUrl}/passenger/passes/monthly/momo-return`;
            ...
            if (!partnerCode || !accessKey || !secretKey) {
                ...
                await WalletTransaction.updateOne({ _id: tx._id }, { $set: { status: "FAILED", note: "Missing MoMo config." } });
                return res.redirect(pageRedirectWithMsg("error", "Thieu cau hinh MoMo tren server.", backQuery));
            }

            const requestId = `${txnRef}-${Date.now()}`;
            const extraData = "";
            const orderInfo = `Thanh toan ve thang ${pad2(month)}/${year}`;
            const rawSignature = [
                `accessKey=${accessKey}`,
                `amount=${price}`,
                `extraData=${extraData}`,
            ...
            const signature = signMomoRaw(rawSignature, accessKey, secretKey);
            const momoPayload = {
                partnerCode,
                accessKey,
                requestId,
            ...
            try {
                const momoData = await createMomoPayment(momoPayload);
                ...
                return res.redirect(momoData.payUrl);
            } catch (momoErr) {
                if (promoReserved) {
                    await releasePromotionReservation(promoReserved._id);
                    await WalletTransaction.updateOne({ _id: tx._id }, { $set: { "rawReturn.promoReleased": true } });
                ...
                await WalletTransaction.updateOne(
                    { _id: tx._id },
                    { $set: { status: "FAILED", note: `MoMo create link failed: ${momoErr.message}` } }
                );
                throw momoErr;
            }
        }
        ...
        await WalletTransaction.updateOne(
            { _id: tx._id },
            { $set: { status: "FAILED", note: `Unsupported payment method: ${paymentMethod}` } }
        );
        return res.redirect(pageRedirectWithMsg("error", "Phuong thuc thanh toan khong hop le.", backQuery));
    } catch (err) {
        console.error("purchaseMonthlyPass:", err);
        const fallbackQuery = {
            passType: parsePassType(req.body.passType),
            paymentMethod: parsePaymentMethod(req.body.paymentMethod),
            routeId: String(req.body.routeId || "").trim(),
            month: parsePositiveInt(req.body.month) || "",
            year: parsePositiveInt(req.body.year) || "",
            promoCode: normalizePromoCode(req.body.promoCode)
        };
        const lowerMessage = String(err?.message || "").toLowerCase();
        let message = err?.message || "Khong the xu ly giao dich.";
        if (lowerMessage.includes("vnpay")) {
            message = "Khong tao duoc lien ket thanh toan VNPAY.";
        } else if (lowerMessage.includes("momo")) {
            message = "Khong tao duoc lien ket thanh toan MoMo.";
        }
        return res.redirect(pageRedirectWithMsg("error", message, fallbackQuery));
    }
};
